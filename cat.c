// taken from github.com/uthcode/learntosolveit/blob/master/languages/cprogs/Ex_8.1_mycat.c
...
int main(int argc, char *argv[])
{
    ...
    else
    {
        ...
        while (--argc > 0)
            if ((fd = open(*++argv, O_RDONLY)) == -1)  error("cat:can't open %s", *argv);
            else { filecopy(fd, STDOUT);  close(fd); }
    }
    return 0;
}
